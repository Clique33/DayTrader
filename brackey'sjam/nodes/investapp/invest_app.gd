...
@export var available_money : float:
	set(value):
		...
		emit_signal("money_moved",available_money)
		if not available_value_frame:
			return
		available_value_frame.get_child(0).text = "Balance : $%.2f" % (available_money)
...
@onready var reward_label: Label = $Chart/RewardLabel
...
var _number_of_stocks_bought : float = 0:
	set(value):
		_number_of_stocks_bought = value
		if not invested_value_frame:
			return
		invested_value_frame.get_child(0).text = "Invested : $%.2f" % (_number_of_stocks_bought*current_value)
var _total_currently_invested : float = 0
...
func _position_buttons() -> void:
	...
	available_value_frame.size = buy_button.size + Vector2(30,0)
...
func _on_buy_button_pressed() -> void:
	if _counter_timeouts >= number_of_time_points:
		return
	_total_currently_invested += available_money
	_number_of_stocks_bought += available_money / current_value
	available_money = 0
...
func _on_sell_button_pressed() -> void:
	...
	if _counter_timeouts >= number_of_time_points:
		return
	available_money += _number_of_stocks_bought * current_value
	var total_made_or_lost : float = available_money - _total_currently_invested
	if total_made_or_lost >= 0:
		reward_label.add_theme_color_override("font_color",Color.from_rgba8(0,194,0))
	else:
		reward_label.add_theme_color_override("font_color",Color.ORANGE_RED)
	reward_label.position = sell_button.position
	reward_label.text = "$%.2f" % total_made_or_lost
	reward_label.show()
	reward_label.get_child(0).start()
	_number_of_stocks_bought = 0
	_total_currently_invested = 0
